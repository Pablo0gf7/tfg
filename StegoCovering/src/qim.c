...
#define DELTA 0.15              // Parámetro de cuantización QIM
...
printf("Extracted message saved to 'mensaje_extraido.txt'\n");

FILE *f = fopen("mensaje_extraido.txt", "wb");
if (!f) {
...
for (int i = 0; i < msg_pos; ++i) {
    ...
    if (bit_count == 8) {
        bit_count = 0;
        // Escribe el carácter en el fichero, no en consola
        ...
        if (strcmp(end_check, "###") == 0) {
            // Opcional: quita los últimos 3 caracteres '###' del fichero
            long pos = ftell(f);
            if (pos >= 3) {
                fseek(f, pos - 3, SEEK_SET);
                fflush(f);
                // Truncar archivo si tu SO lo permite (en Windows o Unix)
                // En C estándar no hay función directa para truncar,
                // pero puedes cerrar y reabrir para truncar,
                // o usar funciones específicas del sistema.
            }
            break;
        }
        c = 0;
