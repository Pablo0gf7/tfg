...
char *extraerMayusculas(const char *cadena)
{
 ...
 int text_embed_cli(const char *msg_path, const char *base_path, const char *out_path) {
    ...
    // Embedding: para cada símbolo en msgBase16, ponemos en mayúscula la siguiente coincidencia en el texto base.
    size_t pos = 0;
    for (size_t j = 0; j < strlen(msgBase16); ++j) {
        char target = msgBase16[j];
        char lower = (char)tolower((unsigned char)target);
        int found = 0;
        for (; pos < L; ++pos) {
            if (tolower((unsigned char)textoMinus[pos]) == lower) {
        ...
        if (!found) {
            fprintf(stderr, "Capacidad insuficiente: no hay suficientes '%c' en el texto base.\n", target);
 ...
 int text_extract_cli(const char *stego_path, const char *out_path) {
    ...
    char *contenido = leerArchivo(fin);
    cerrarArchivo(fin);
    if (!contenido) { fprintf(stderr, "No se pudo leer %s\n", stego_path); return 1; }

    char *mayus = extraerMayusculas(contenido);
    free(contenido);
    if (!mayus) { fprintf(stderr, "extraerMayusculas() falló\n"); return 1; }

    char *mensaje = decodificarMsg(mayus);
    free(mayus);
